...
class HillClimbing(BaseOptimizer):
    ...
    def optimize(self):
        # 随机初始化 - 使用连续值而不是二进制
        current = np.random.uniform(-1, 1, self.sequence_length)
        current_fitness = self.evaluate(current)

        while not self.should_stop():
            # 生成邻居 - 添加高斯噪声
            neighbor = current.copy()
            noise_pos = np.random.randint(0, self.sequence_length)
            neighbor[noise_pos] += np.random.normal(0, 0.1)
            # 确保在边界内
...
class GeneticAlgorithm(BaseOptimizer):
    ...
    def optimize(self):
        # 初始化种群 - 使用连续值
        population = [np.random.uniform(-1, 1, self.sequence_length) for _ in range(self.pop_size)]
        ...
        while not self.should_stop():
            ...
            for _ in range(self.pop_size):
                ...
                winner_idx = int(tournament_indices[np.argmax([fitness_scores[i] for i in tournament_indices])])
            ...
            for i in range(0, self.pop_size-1, 2):
                ...
                for j in range(self.sequence_length):
                    if np.random.random() < 0.01:  # 变异概率
                        ...
                        new_population[i][j] = np.clip(new_population[i][j], -1, 1)
                    if i+1 < len(new_population) and np.random.random() < 0.01:
                        new_population[i+1][j] += np.random.normal(0, 0.1)
                        new_population[i+1][j] = np.clip(new_population[i+1][j], -1, 1)
...
class DifferentialEvolution(BaseOptimizer):
    ...
    def optimize(self):
        # 初始化种群（连续值）
        population = np.random.uniform(-1, 1, (self.pop_size, self.sequence_length))
        ...
        for i in range(self.pop_size):
            if self.should_stop():
                break
            fitness = self.evaluate(population[i])
...
class ParticleSwarmOptimization(BaseOptimizer):
    ...
    def optimize(self):
        # 初始化粒子群
        particles = np.random.uniform(-1, 1, (self.swarm_size, self.sequence_length))
        velocities = np.random.uniform(-0.1, 0.1, (self.swarm_size, self.sequence_length))
        ...
        while not self.should_stop():
            for i in range(self.swarm_size):
                ...
                # 评估
                fitness = self.evaluate(particles[i])
...
class HybridPSOGWO(BaseOptimizer):
    ...
    def optimize(self):
        # 初始化种群
        population = np.random.uniform(-1, 1, (self.pop_size, self.sequence_length))
        velocities = np.random.uniform(-0.1, 0.1, (self.pop_size, self.sequence_length))
        ...
        alpha_pos = population[int(sorted_indices[0])].copy()
        beta_pos = population[int(sorted_indices[1])].copy()
        delta_pos = population[int(sorted_indices[2])].copy()
