...
path1 = r"C:\Users\broth\Desktop\whole_speed.csv"
ImuDataTxt_OCT= open(path1,"w")
path2 = r"C:\Users\broth\Desktop\tentacals.csv"
ImuDataTxt_tentacles= open(path2,"w")
#计算任意维度欧氏距离
...
class PSO:
    ...
    def optimize(self):
        global Found,max_range,min_range,real_min_value
        # 初始化粒子位置和速度
            ...
            for i in range(self.swarm_size):
                ...
                if self.cost_func(swarm[i]) < p_best_val[i]:
                    ...
                    if p_best_val[i] < self.g_best_val:
                        ...
                        if math.fabs(self.g_best_val)<1e-9:################
                            print("已找到,适应度为：",self.g_best_val-real_min_value)
                            Found = 1
                            break
...
class PSOControl:
    ...
    def run_pso(self):
        ...
        for i, params in enumerate(self.params_list):
            ...
            #print("第", i, "个触手位置", pso.g_best_pos,"size:",pso.swarm_size, "迭代次数",pso.max_iter,"找到的值", pso.g_best_val)
            # 记录每个PSO的最优解和最优值
            self.best_values[i] = best_value
            self.best_positions[i] = best_position
           # print(best_value)
            ImuDataTxt_tentacles.write(str(best_position)+",")
...
class Controller:
    ...
    def run_psocontrol(self):
        ...
        for j in range(self.num_iteration):
            ...
            if Found == 1:
                ImuDataTxt_tentacles.write(str(COUNT)+",")
...
#1群体4章鱼20触手，群体迭代10次，，一触手20吸盘迭代100次： 10*100 = 1000
                                                     #20*20 = 400
