...
path1 = r"C:\Users\broth\Desktop\wolf_whole_accuracy.csv"
    ...
    def run_pso(self):
        for i, params in enumerate(self.params_list):
            ...
            if Found == 1:
                break
    #根据运行结果，群体信息进行调整
    def adjust_pso(self,paradise,paradise_position):
        global min_range,max_range
        ...
        for i, params in enumerate(self.params_list):
            ...
            move = top_to_one(self.best_values,2)
            ...
            if paradise == 1 and move[i] == 1:
                center = self.best_positions[i] + np.random.uniform(-1, 1, size=self.dim) * (self.best_positions[i] - paradise_position)
            else:
...
class Controller:
    ...
    def run_psocontrol(self):
        for j in range(self.num_iteration):
            ...
            for i in range(self.num_control):
                self.PSOControls[i].adjust_pso(adjust[i],self.best_position_all)
