...
path1 = r"C:\Users\broth\Desktop\whole_evaluate_accuracy.csv"
...
class PSO:
    ...
    def optimize(self):
        global min_range, max_range, real_min_value, Found
        # 初始化粒子位置和速度
        ...
        for t in range(self.max_iter):
            for i in range(self.swarm_size):
                ...
                if self.cost_func(swarm[i]) < p_best_val[i]:
                    ...
                    if self.g_best_val == real_min_value :
                        print("found")
                        Found = 1
                        break
            if Found == 1:
...
class PSOControl:
    ...
    def adjust_pso(self,paradise):
        ...
        #1.mean_value = mean(self.best_values)
        ...
        for i, params in enumerate(self.params_list):
            ...
            if best_value_iteration != worst_value_iteration:
                ...
                #1. self.best_values_ratio[i] = ( self.best_values_ratio[i] - mean_value)/(best_value_iteration - worst_value_iteration) 均值归一化（-1,1）
                #1. self.best_values_ratio[i] = (self.best_values_ratio[i] + 1)*(1.3-0.7)/(1+1) - 1
                #2. self.best_values_ratio[i] = math.atan(self.best_values_ratio[i]) / (math.pi / 2) 反正切归一化（-1,1）
                #2. self.best_values_ratio[i] = (self.best_values_ratio[i] + 1)*(1.3-0.7)/(1+1) - 1
...
#################################
    ...
    for i in range(17):
        for j in range(num_ex):
            ...
            print("###################第", j, "次", "time:", time_cost, "s")
            print("###################第", j, "次", "fitness:", controller.best_value_all)
