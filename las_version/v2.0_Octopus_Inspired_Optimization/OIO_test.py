...
class Controller:
    ...
    def run_psocontrol(self):
        ...
        for j in range(self.num_iteration):
            threads = []
            for i in range(self.num_control):
                thread = threading.Thread(target=run_octopus, args=(i,))
...
#################################
    ...
    COUNT = 0  #
    particle_count = 0
    position_record_arr = np.empty((num_tentacles*num_control*1000, len(cost_fun), dim))  # (4*5*50),只记录最终位置
    ...
    row = 0##
    column = 0#
    fitness_record_array = np.full((num_tentacles*num_control*200*num_iteration, len(cost_fun)), -1.23456)#
    #fitness_record_array = np.full((num_iteration, len(cost_fun)),-1.23456)

    print(num_tentacles*num_control*200*num_iteration)
    for i in range(18):
        print("###########################################################")
        Count_i = i
        record_fitness = float('inf')
        ...
        iter_count = 0
        END = 0
        position_record_arr = np.empty((num_control*num_tentacles * 1000, len(cost_fun), dim))
